...
const rules = {
  ...
  'targetNode is the second child of root': info =>
    info.currentTreeRootSecondChildExcludingDragging === info.targetNode,
  'currentTree existed': info => info.currentTree,
  'currentTree empty': info =>
    !findChild(info, info.currentTree.rootData.children, v => v),
  ...
  'targetNode has children excluding placeholder': info =>
    findChild(info, info.targetNode.children, v => v !== info.dplh),
  'targetNode is 1st child': info =>
    findChild(info, info.targetNode.parent.children, v => v) ===
    info.targetNode,
  'targetNode is last child': info =>
    findChild(info, info.targetNode.parent.children, v => v, true) ===
    info.targetNode,
  'on targetNode middle': info => info.offset.y <= info.tiMiddleY,
  'at left': info => info.offset.x < info.tiOffset.x,
  'at indent right': info =>
    info.offset.x > info.tiOffset.x + info.currentTree.indent,
}
...
export default function autoMoveDragPlaceHolder(draggableHelperInfo) {
  ...
  attachCache(info, new Cache(), {
    ...
    // tree
    currentTree() {
      const currentTree = getTreeByPoint(
        this.offsetToViewPort.x,
        this.offsetToViewPort.y,
        trees,
      )
      if (currentTree) {
    ...
    currentTreeRootSecondChildExcludingDragging() {
      return this.currentTree.rootData.children
        .slice(0, 3)
        .filter(v => v !== this.node)[1]
    },
    // placeholder
    dplhEl() {
    ...
    targetNode() {
      ...
      while (true) {
        ...
        const innerEl = document
          .getElementById(currentNode._id)
          .querySelector('.tree-node-inner')
    ...
    if (exec('targetNode is placeholder') === false) {
      if (exec('targetNode is the second child of root') === true) {
        if (exec('targetNode has children excluding placeholder') === false) {
          ...
          exec('targetNode has children excluding placeholder') === true
        ) {
        ...
        if (exec('currentTree empty') === false) {
          if (exec('targetNode at top') === true) {
            if (exec('placeholder in currentTree') === true) {
              if (
                exec('targetNode has children excluding placeholder') === false
              ) {
            ...
            if (exec('targetNode at bottom') === false) {
              if (exec('placeholder at top') === true) {
                ...
                if (
                  ...
                  exec('targetNode has children excluding placeholder') ===
                  false
                ) {
